...
\KKvScanOn\KKvUsePreset{LuaStyle}
\subsection{The \KKverb|word_boundary| Option}
The option controls how the scanner identifies the limits of a keyword during the tokenization process. 
It determines whether a sequence of characters should be treated as a standalone keyword or as part of a larger string.

\begin{description}
  \item[When Disabled: \KKverb|false|] The scanner performs a raw string search. Any occurrence of the target token is highlighted, regardless of its surrounding characters. For example, if the keyword is \KKverb|if|, it will be highlighted even inside the word \KKverb|different|. 
  \item[When Enabled (Default): \KKverb|true|] \KKvUsePreset{TeXStyle}This mode prevents unintended partial matching. It ensures that a keyword is highlighted only when it stands as a complete word. Crucially, symbols (such as \texttt{(}, \texttt{[}, \texttt{\#}, etc.) are automatically treated as boundaries. For example, \KKverb|\def#1| will result in \KKverb|\def| being highlighted, whereas the \KKvUsePreset{LuaStyle}\KKverb|if| inside \KKverb|different| will remain plain text.
\end{description}
