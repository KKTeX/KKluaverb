...
\bigskip\begin{center}
  ...
  \begin{tblr}{
    ...
    wrap & \KKverb|false| & Enables auto wrapping in code blocks. (Since v2.3.0) \\
    wrapindent & \KKverb|2em| & Continuation line indent. (Since v2.3.0) \\
  \end{tblr}
\end{center}\bigskip
...
\begin{itemize}
  ...
  \item The delimiter characters of \KKverb|\KKverb| have no special meaning here (since v2.2.0): the environment is terminated only by \KKverb|\KKcodeE|.
\end{itemize}
...
Since v2.2.0, the raw scan remembers which command started it. A scan started by \verb|\KKverb| is terminated only by its closing delimiter, and a scan started by \verb|\KKcodeS| is terminated only by \verb|\KKcodeE|. Therefore you can now write \verb|\KKcodeS| and \verb|\KKcodeE| in the argument of \verb|\KKverb|, and the delimiter characters of \verb|\KKverb| can appear freely inside the \verb|\KKcodeS/E| environment.
(In versions prior to v2.2.0, \verb|\KKcodeS| forcibly started the encoding system even when it was wrapped by \verb|\KKverb|, and the \verb|\KKverb| delimiter terminated the \verb|\KKcodeS/E| environment.)
Note that \verb|\KKcodeS| appearing outside of a raw scan still starts the encoding system unconditionally; if you need to display it as it is in a normal context, use \verb|\KKverb| or deactivate the scanner by \verb|\KKvScanOff|.
...
Since v2.2.0, you can output a literal \verb|\KKcodeE| inside the environment by putting the escape character (default: a backslash) immediately before it. The escape character itself is removed from the output, and the raw scan continues.
...
\end{OutPut}
\KKvScanOff
The escape character can be changed by \verb|\KKvSetCodeEscape{<character>}|.
\KKvScanOn

\subsubsection{Note\#4: Automatic Line Wrapping (Since v2.3.0)}
\KKvScanOff
By default, long lines in \verb|\KKcodeS/E| blocks overflow beyond the right margin (Overfull \verb|\hbox|). Since v2.3.0, you can enable automatic wrapping by \verb|\KKvOpChange{wrap=true}|.
...
The indentation of continuation lines equals the original code indentation plus \verb|wrapindent| (default: \verb|2em|). For style=2 (line-numbered), continuation lines do not receive a line number.
\KKvScanOn
...
\KKvScanOff
\subsection{TeX-command Mapping (Since v2.3.0)}
\verb|\KKvSetMap| replaces a character with another \emph{character}. However, there are cases where you want to replace a character with a \emph{TeX command}. For example, \verb|\KKvSetMap{ }{␣}| replaces spaces with U+2423, but this character may be rendered as a full-width glyph in Japanese font contexts.

\verb|\KKvSetMapTeX{<from>}{<tex-cmd>}| solves this by mapping the character to a TeX command, which is emitted via \verb|tex.sprint| (with expansion) at output time.
\KKvScanOn

\begin{SourceCode}{Input}{TeX}[Actual Use]
  \KKvSpaceVisible              % ON: spaces -> \textvisiblespace
  \KKverb|  Hello World!  |

  \KKvClearMapTeX{ }            % OFF: back to NBSP
  \KKverb|  Hello World!  |
\end{SourceCode}
\KKvScanOn
\begin{OutPut}{Output}
...
\KKvScanOff
\verb|\KKvSpaceVisible| is a convenience wrapper for \verb|\KKvSetMapTeX{ }{\textvisiblespace}|.
\verb|\KKvClearMapTeX{<char>}| removes the TeX-command mapping for a character and restores the default behaviour (for space: reverts to non-breaking space).
Note that calling \verb|\KKvSetMap| on a character that currently has a TeX-command mapping automatically removes the old mapping.
\KKvScanOn
\end{document}
